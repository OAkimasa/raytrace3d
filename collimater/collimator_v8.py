...
ax.set_title(
    "Converting F2 Light to ≤ 7.5 mm Parallel Beam")
aspect_X = 3
aspect_Y = 1
aspect_Z = 1
...
lens_shift = np.array([70., 0., 0.])
surface_1 = [np.array([0.00, -7.5, 0.])+lens_shift, [0., 1., 0.], 25.4, -7.5*2]  # off-axis parabola mirror

evaluation_plane = [np.array([-70.000, 0., 0.])+lens_shift, [1., 0., 0.], 35/2, 0.]  # air->air
check_plane = [np.array([0., 0., 0.])+lens_shift, [1., 0., 0.], 12.7, 0.]  # air->air
...
VF.plot_parabola(surface_1)  # レンズ描画
...
pointsz, pointsx = np.meshgrid(
    np.arange(-width+rayCenterZ, width+rayCenterZ+space, space),
    np.arange(-width+rayCenterX, width+rayCenterX+space, space))
pointsY = np.array([rayCenterY]*size)
pointsZ = pointsz.reshape(size)*rayDensity
pointsX = pointsx.reshape(size)*rayDensity
raySPoint0 = VF.make_points(pointsX, pointsY, pointsZ, size, 3)

bool_list = []
for i in range(len(raySPoint0)):  # 入射瞳に含まれるかどうかの判定
    if (raySPoint0[i][0]-rayCenterX)**2 + (raySPoint0[i][1]-rayCenterY)**2 + (raySPoint0[i][2]-rayCenterZ)**2 <= (7.5/2)**2:
        bool_list.append(True)
    # if raySPoint0[i][1]**2 + raySPoint0[i][2]**2 <= lens_unit_aperture_R**2:
        # bool_list.append(True)
    else:
...
aperture_plane = np.array([[-5., 0., 0.], [1., 0., 0.], 2.5, np.inf], dtype=object)  # 絞り面
ray_start_pos_init = raySPoint0  # 初期値
# ray_start_dir_init = np.array([0,0,0])-raySPoint0  # F2
ray_start_dir_init = np.array([[0.0, 1.0, 0.0]]*len(raySPoint0))  # 平行光
...
#     VF.ray_end_dir = VF.ray_start_dir  # 絞り面までの光線方向を保持
...
for i, VF in enumerate(VF_list):
    ...
    print(VF.ray_start_pos)
    print(VF.ray_start_dir)
    VF.set_surface(surface_1)  # surface_1をVFに登録
    # VF.raytrace_plane()  # 光線追跡
    VF.raytrace_parabola()  # 光線追跡
    VF.reflect()  # 反射
plot_line_CdF()  # 光線描画
...
#                  [1., 0., 0.], 25.4/4, -R_1]  # air->lens1
#     surface_2 = [[lens_pos+t_c1, 0., 0.]+lens_shift,
#                  [1., 0., 0.], 25.4/4, -R_2]  # lens1->air
#     evaluation_plane = [[f_b+t_c1-0.128, 0., 0.],
#                         [1., 0., 0.], 24, 0.]  # air->air
# else:
#     surface_1 = [[lens_pos+0.00, 0., 0.]+lens_shift,
#                 [1., 0., 0.], 25.4/4, R_2]  # air->lens1
#     surface_2 = [[lens_pos+t_c1, 0., 0.]+lens_shift,
#                 [1., 0., 0.], 25.4/4, R_2]  # lens1->air
#     evaluation_plane = [[35.902, 0., 0.],
#                         [1., 0., 0.], 24, 0.]  # air->air
# check_plane = [[f_b+t_c1, 0., 0.], [1., 0., 0.], 25.4/4, 0.]  # air->air
# lens_list = [surface_1, surface_2]
# plane_list = [evaluation_plane, check_plane]

# # for lens in lens_list:
# #     VF.plot_lens(lens)  # レンズ描画
# VF.plot_mirror(surface_1)  # レンズ描画
# VF.plot_lens(surface_2)  # レンズ描画
...
#                    refractive_index_after=N_BK7_list[i])  # surface_1をVFに登録
...
#                    refractive_index_before=N_BK7_list[i],
...
# chief_ray_OPD_list = []
# for n_1 in N_BK7_list:
#     chief_ray_OPD = np.abs(rayCenterX) + t_c1*n_1 + \
#         np.abs(evaluation_plane[0][0]-surface_2[0][0])
